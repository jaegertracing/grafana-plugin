...
2. Runs `examples/reverse-proxy/test.sh` — 12 curl/jq assertions covering the proxy layer (HTTP responses, `data-inject-target="BASE_URL"` presence, `/api/v3/services`, assets) and Grafana datasource provisioning (`url` field).
3. Runs Playwright tests (`tests/reverse-proxy.spec.ts`) against the stack — 6 assertions covering `/api/v3/services` reachability via the datasource URL, datasource `url` provisioning, and the config page loading correctly.
